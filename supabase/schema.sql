...
-- NOTE: the waitlist feature (and its `waitlist` table, previously defined
-- here) has been retired from the app — Telos is now taking clients
-- directly. This file is a from-scratch setup script, so removing it here
-- does not touch the live table or any signups already stored in it; drop
-- that table by hand in the Supabase dashboard if it is no longer wanted.
